...
from moo_algorithm.pfg_moea import run_pfgmoea, run_pfgmoea_mix
from moo_algorithm.nsga_iii import run_nsga_iii
from moo_algorithm.moead import run_moead, init_weight_vectors_3d
import json, os, time
from perm_operator import cx_crossover, ox_crossover
...
if __name__ == "__main__":
    ...
    for data_file in data_files:
        ...
        base_path = os.path.join("result_crossover", f"{number_customer}customers")
        ...
        for i in range(pop_size):
            indi = init_random(problem, 0.7)
            indi_list.append(indi)

        crossover_list = [cx_crossover, ox_crossover, crossover_PMX]
        
        ##### PFGMOEA Mix ############
        pfg_start = time.time()
                # Khởi tạo quần thể ban đầu
        pfg_history = run_pfgmoea_mix(processing_number, problem, indi_list, pop_size, max_gen,
                                  G, sigma, crossover_list, mutation_flip, crossover_rate, mutation_rate, cal_fitness)
        pfg_end = time.time()
        pfg_result = {'time': pfg_end - pfg_start, 'history': pfg_history}
        pfg_path = os.path.join(base_path, "mix", file_name)
        ...
        pfg_history = run_pfgmoea(processing_number, problem, indi_list, pop_size, max_gen,
                                  G, sigma, ox_crossover, mutation_flip, crossover_rate, mutation_rate, cal_fitness)
        pfg_end = time.time()
        pfg_result = {'time': pfg_end - pfg_start, 'history': pfg_history}
        pfg_path = os.path.join(base_path, "ox", file_name)
        os.makedirs(os.path.dirname(pfg_path), exist_ok=True)
        with open(pfg_path, 'w') as f:
            json.dump(pfg_result, f)
        ...
        for i in range(pop_size):
            ...
            indi_list.append(indi)
        pfg_history = run_pfgmoea(processing_number, problem, indi_list, pop_size, max_gen,
                                  G, sigma, cx_crossover, mutation_flip, crossover_rate, mutation_rate, cal_fitness)
        pfg_end = time.time()
        pfg_result = {'time': pfg_end - pfg_start, 'history': pfg_history}
        pfg_path = os.path.join(base_path, "cx", file_name)
        os.makedirs(os.path.dirname(pfg_path), exist_ok=True)
        with open(pfg_path, 'w') as f:
            json.dump(pfg_result, f)
        ...
        pfg_end = time.time()
        pfg_result = {'time': pfg_end - pfg_start, 'history': pfg_history}
        pfg_path = os.path.join(base_path, "pmx", file_name)
        os.makedirs(os.path.dirname(pfg_path), exist_ok=True)
        with open(pfg_path, 'w') as f:
            json.dump(pfg_result, f)